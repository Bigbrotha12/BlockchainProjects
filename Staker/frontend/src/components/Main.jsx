...
const Main = (props) => {
  ...
  const enableW3 = async () => {
    console.log("Enabling Web3...");
    setWeb3(await Moralis.enableWeb3());
    if(web3){
      setContract(new web3.eth.Contract(abi, stakeContractAddress));
      setStakeCoin(new web3.eth.Contract(tokenAbi, stakeCoinAddress));
      setRewardCoin(new web3.eth.Contract(tokenAbi, rewardCoinAddress));
      setAccount(await web3.eth.getAccounts());
    }
  };

  const stake = (amount) => {
    if(web3 && stakeCoin && contract) {
  ...
  const activateReward = (amount, period) => {
    if(web3 && contract) {
