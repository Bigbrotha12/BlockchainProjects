...
contract Staking is Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice Allows admin to withdraw unclaimed tokens after reward program ends.
    /// @dev After withdrawing unclaimed tokens, users will no longer be able claim
    /// their rewards. Can be undone by calling AddFunding to replenish funds and allow
    /// certain users to claim once again.
    function withdrawUnclaimed() external onlyOwner {
        require(
            block.number >= _endBlock,
            "Current rewards program must be complete before withdrawing unclaimed tokens"
        );
        uint256 unclaimedBalance = _rewardsToken.balanceOf(address(this));
        _rewardsToken.safeTransfer(msg.sender, unclaimedBalance);
    }
}
